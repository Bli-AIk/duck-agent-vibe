...
export function formatTeachingHandoff(candidate: TeachingCandidate): string {
  ...
  return [
    ...
    "教学流程：先调用 duck_context7 查询这个库的一个主题；然后只回答当前疑问，给出原始文档链接、短原文摘录和极简中文解释。若用户明确要求“样板、示例、文档原文、我看看”，可以展示 Context7 或源码返回的短代码原文，并标注“原文，仅供对照”；不要让用户直接粘贴。不要生成用户项目补丁、改写后的完整实现或可交付代码；除非用户明确要求详细，否则不超过 80 个汉字、最多 3 句。答疑优先，不要另列实践清单。文档不足时先查本机源码；需要上游仓库时先询问是否允许克隆到项目外的临时目录。",
  ].join("\n");
}
